...
<section class="khungaddm d-flex">
    <div class="khungtables">
        <h1 class="tieude" style="text-align: center;">THỐNG KÊ SẢN PHẨM THEO DANH MỤC</h1>
        <table class="table">
            <tr>
                <th>TÊN DANH MỤC</th>
            ...
            <?php
            $totalProducts = 0; // Biến để tính tổng số lượng sản phẩm
            foreach ($listthongke as $thongke) : 
                $totalProducts += $thongke['product_count']; // Tính tổng số lượng sản phẩm
            ?>
                <tr>
                    <td><?php echo $thongke['name']; ?></td>
                    <td><?php echo $thongke['product_count']; ?></td>
                </tr>
            <?php endforeach; ?>
            <tr>
        ...
        function drawChart() {
            var data = google.visualization.arrayToDataTable([
                ...
                <?php foreach ($listthongke as $thongke) : ?>
                    // Kiểm tra xem có tên danh mục và số lượng sản phẩm không
                    <?php if (isset($thongke['name']) && isset($thongke['product_count'])) : ?>['<?php echo $thongke['name']; ?>', <?php echo $thongke['product_count']; ?>],
                    <?php endif; ?>
                <?php endforeach; ?>
            ]);
